...
if _QT_AVAILABLE:
    ...
    # Headless stub — web backend does not use RefreshLoop (it fetches bars on-demand)
    class RefreshLoop:
        """Headless stub: web backend fetches K-lines on-demand, not via loop."""

        frame_ready = None   # type: ignore[assignment]
        status_changed = None  # type: ignore[assignment]

        def __init__(self, data_source=None, n_bars=None, interval_ms=None,
                     cancel_token=None, parent=None, **__kwargs):
            self._source = data_source
            self._n_bars = n_bars
            self._interval_ms = interval_ms
            self._cancel_token = cancel_token
